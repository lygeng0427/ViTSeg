...
def main_worker(rank: int, world_size: int, args: argparse.Namespace) -> None:
    ...
    sv_path = os.path.join('./0CV_reports_5', sv_path)
    ensure_path(sv_path)
...
def exp_validate(args: argparse.Namespace, val_loader: torch.utils.data.DataLoader, model: DDP, sv_path) -> Tuple[torch.tensor, torch.tensor]:
    ...
    for run in range(args.n_runs):
        ...
        rep = 0
        freq = 100 if not args.debug else 100
        ...
        log(epi_novel_IoU[:rep])
        log(epi_base_IoU[:rep])
        plt.figure(figsize=(10,10))
...
if __name__ == "__main__":
    ...
    if args.test_all:
        exp_name = ["resnet34_upernet", "resnet34_None", "resnet50_upernet", "resnet50_None"]
        arch = ["resnet","resnet","resnet","resnet",]
        layers = [34,34,50,50]
        seghead = ["UPerNet","None","UPerNet","None"]
        # exp_name = ["resnet34_None", "resnet50_None"]
        # arch = ["resnet","resnet"]
        # layers = [34,50]
        # seghead = ["None","None"]
        for i in range(4):
            args.exp_name = exp_name[i]
            args.arch = arch[i]
            args.layers = layers[i]
            args.seghead = seghead[i]
