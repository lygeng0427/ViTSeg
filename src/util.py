...
def load_base_weight(decoder, base_model, args):
    if args.arch == 'resnet':
        decoder.conv.weight.data[:args.num_classes_tr] = base_model.classifier.weight.data  # [num_cls, 512, 1, 1]
        decoder.conv.bias.data[:args.num_classes_tr] = base_model.classifier.bias.data

    elif args.arch in ['dino', 'dinov2']:
        if args.decoder == 'linear':
            decoder.head.weight.data[:args.num_classes_tr] = base_model.decoder.head.weight.data  # [num_cls, 768]
            decoder.head.bias.data[:args.num_classes_tr] = base_model.decoder.head.bias.data  # [num_cls]
        elif args.decoder == 'mask_transformer':
            base_state_dict = base_model.decoder.state_dict()

            for name in ['cls_emb'] + (['mask_norm.weight', 'mask_norm.bias'] if args.decoder_cfg.mask_norm else []):
                new_param_data = decoder.state_dict()[name]
                if name == 'cls_emb':
                    new_param_data[:, :args.num_classes_tr] = base_state_dict[name]
                else:
                    new_param_data[:args.num_classes_tr] = base_state_dict[name]
                base_state_dict[name] = new_param_data

            decoder.load_state_dict(base_state_dict, strict=True)

    return decoder


def load_pretrain_weight(model, args):

    if args.ckpt_full_path is not None:
        filepath = args.ckpt_full_path
    else:
