...
def compute_wce(one_hot_gt, n_novel):
    ...
    wce[:, :, 0, :, :] = 0.01 if shot == 1 else 0.01  # Increase relative coef of novel classes if labeled samples are scarce
    return wce
...
def visualize(img,svpath,img_id):
    invTrans = transforms.Compose([transforms.Normalize(mean=[0., 0., 0.], std=[1/0.229, 1/0.224, 1/0.225]),
                               transforms.Normalize(mean=[-0.485, -0.456, -0.406], std=[1., 1., 1.]),
                               ])
    inv_q = invTrans(img)
    plt.imsave(svpath+"/"+img_id, inv_q.permute(1, 2, 0).numpy())
    return
