...
def main_worker(rank: int, world_size: int, args: argparse.Namespace) -> None:
    ...
    # ========== Test  ==========
    episodic_validate(args=args, val_loader=val_loader, model=model)
    # cleanup()


def episodic_validate(args, val_loader, model, use_callback=False):
    ...
    for run in range(args.n_runs):
        ...
        for _ in tqdm(range(nb_episodes), leave=True):
            ...
            with torch.no_grad():
                ...
                q_img = q_img.to(device, non_blocking=True)             # [B, 3, 448, 448]
                q_label = q_label.to(device, non_blocking=True)         # [B, 448, 448]
                q_valid_pix = q_valid_pix.to(device, non_blocking=True) # [B, 448, 448]
                q_feat = model.extract_features(q_img)                  # [B, 1024, 768]
                ...
                if args.generate_new_support_set_for_each_task:
                    ...
                    s_imgs = s_imgs.to(device, non_blocking=True)       # [5*shot, 3, 448, 448]
                    s_label = s_label.to(device, non_blocking=True)     # [5*shot, 448, 448]
                    s_feat = model.extract_features(s_imgs)  # dino & PSPNet output different shapes # [5*shot, 1024, 768]
            ...
            decoder.train()
            train_decoder(decoder, s_feat, s_label.clone(), iterations=args.adapt_iter, lr=0.01, args=args)
    ...
    return (base_mIoU.mean() + novel_mIoU.mean())/2, 1
