...
from src.test_all import episodic_validate
...
from .util import ensure_path, set_log_path, log, load_base_weight
import argparse
...
def main(args: argparse.Namespace) -> None:
    ...
    model_name = args.arch + str(args.layers) if args.arch == 'resnet' else args.arch
    sv_path = 'pretrain_{}/{}/split{}_shot{}/{}'.format(
        args.data_name, model_name, args.split, args.shot, args.exp_name)
    ...
    if args.arch == 'resnet':
        ...
        param_list = [{'params': model.decoder.parameters(), 'lr': args.lr * args.scale_lr}]
        if args.tune_backbone:
            param_list += [{'params': model.backbone.parameters(), 'lr': args.lr}]
        optimizer = get_optimizer(args, param_list)
    ...
    for epoch in range(args.epochs):
        ...
        # intersections = torch.zeros(args.num_classes_tr, device=device)
        # unions = torch.zeros(args.num_classes_tr, device=device)
        ...
        for i in range(1, iter_per_epoch+1):
            ...
            if i % args.log_freq == 0:
                ...
                mIoU = (intersection[idx] / (union[idx] + 1e-10)).mean()
...
def standard_validate(args, val_loader, model, use_callback):
    ...
    base_mIoU = (intersections[1:] / (unions[1:] + 1e-10)).mean()
    bg_IoU = intersections[0] / (unions[0] + 1e-10)
    acc = intersections.sum() / unions.sum()
    log(f'Testing results: running loss {loss_meter.avg:.2f}, Acc {acc:.4f}, mIoU {mIoU:.4f}, base mIoU {base_mIoU:.4f} BG IoU {bg_IoU:.4f}\n')
