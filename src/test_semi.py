...
from src.classifier_semi import SemiClassifier, train_semi, get_qsuedo_label, calc_mask_accuracy
...
def main_worker(rank: int, world_size: int, args: argparse.Namespace) -> None:
    ...
    if args.ckpt_used is not None:
        ...
        filepath = '/scratch/yz5160/DIaM/model_ckpt/1/pspnet_resnet50/model.pth'
    ...
    # ========== Test  ==========
    validate(args=args, semi_val_loader=semi_val_loader, model=model, )
    cleanup()


def validate(args: argparse.Namespace, semi_val_loader: torch.utils.data.DataLoader, model: DDP) -> Tuple[
    ...
    nb_episodes = len(semi_val_loader) if args.test_num == -1 else int(args.test_num / args.batch_size_val)
    ...
    for run in range(args.n_runs):
        ...
        features_cls_s, gt_cls_s, features_semi_s, gt_semi_s = None, None, None, None
        if not args.generate_new_support_set_for_each_task:
            with torch.no_grad():
                ...
                cls_s_imgs, cls_s_label, semi_s_imgs, semi_s_label, semi_s_path_list = semi_val_loader.dataset.generate_support(
                    [], remove_them_from_query_data_list=True)
                nb_episodes = len(
                    semi_val_loader) if args.test_num == -1 else nb_episodes  # Updates nb_episodes since some images were removed by generate_support
                cls_s_imgs = cls_s_imgs.to(device, non_blocking=True)
                cls_s_label = cls_s_label.to(device, non_blocking=True)
                semi_s_imgs = semi_s_imgs.to(device, non_blocking=True)
                semi_s_label = semi_s_label.to(device, non_blocking=True)
                # pdb.set_trace()
                # imgs = [np.array(Image.open(fname)) for fname in semi_s_path_list]
                # imgs = np.array(imgs)
                # dino_feature_extractor = ViTFeatureExtractor.from_pretrained('facebook/dino-vitb8')
                # model = ViTModel.from_pretrained('facebook/dino-vitb8')
                # dino_features = dino_feature_extractor(images=semi_s_imgs[0], return_tensors="pt")
                features_cls_s = model.extract_features(cls_s_imgs).detach().view(
                    (args.num_classes_val, args.cls_shot, c, h, w))
                gt_cls_s = cls_s_label.view((args.num_classes_val, args.cls_shot, args.image_size, args.image_size))
                features_semi_s = model.extract_features(semi_s_imgs).detach().view(
                    (args.num_classes_val, args.semi_shot, c, h, w))
                # gt_semi_s = semi_s_label.view((args.num_classes_val, args.semi_shot, args.image_size, args.image_size))
                gt_semi_s = semi_s_label.unsqueeze(1)

        for _ in tqdm(range(nb_episodes), leave=True):
            t0 = time.time()

            # '''Use n_shot support  image to train a classifier for pseudo-labelling'''
            # classifier_semi = SemiClassifier(args,list(features_cls_s.size()))

            # train_semi(classifier_semi,features_cls_s,gt_cls_s,args.num_classes_val)

            # '''pseudo-labelling'''
            # semi_s_pseudo = get_qsuedo_label(classifier_semi,features_semi_s,list(gt_cls_s.size())) #[num_novel_classes * shot,473,473]
            # gt_semi_s = semi_s_pseudo.view((args.num_classes_val, args.semi_shot, args.image_size, args.image_size))
            with torch.no_grad():
                try:
                    loader_output = next(iter_loader)
                except (UnboundLocalError, StopIteration):
                    iter_loader = iter(semi_val_loader)
                    loader_output = next(iter_loader)
                ...
                features_q = model.extract_features(qry_img).detach().unsqueeze(1)
            ...
            '''pseudo-labelling'''
            semi_s_probas, pseudo_semi_s_label = get_qsuedo_label(classifier_semi, features_semi_s, list(
                gt_cls_s.size()))  # [num_novel_classes * shot,473,473]
            pseudo_gt_semi_s = pseudo_semi_s_label.view(
                (args.num_classes_val, args.semi_shot, args.image_size, args.image_size))
            '''calculate pseudo-label meanIOU'''
            # pdb.set_trace()
            calc_mask_accuracy(semi_s_probas, gt_semi_s, args.num_classes_val + 1,
                               semi_val_loader.dataset.novel_class_list)

            '''Jaccard for double test above calculated IOU'''
            # semi_s_label_jac = semi_s_label.clone()
            # semi_s_label_jac[torch.where(semi_s_label_jac == 255)] = 0
            # semi_s_label_jac[torch.where(semi_s_label_jac!=0)] = semi_s_label_jac[torch.where(semi_s_label_jac!=0)]-15
            # jacncard = JaccardIndex(task="multiclass", num_classes=args.num_classes_val+1)
            # jacncard = jacncard.cuda()
            # print('jacncard MeanIOU',jacncard(pseudo_semi_s_label, semi_s_label_jac).item())
            # pdb.set_trace()

            # =========== Initialize the classifier and run the method ===============
            '''concatenate label and unlabel data'''
            features_s = torch.cat((features_cls_s, features_semi_s), 1)
            gt_s = torch.cat((gt_cls_s, pseudo_gt_semi_s), 1)

            # features_s = features_cls_s
            # gt_s = gt_cls_s
            base_weight = model.classifier.weight.detach().clone().T
            base_bias = model.classifier.bias.detach().clone()
            ...
            classifier.compute_pi(features_q, valid_pixels_q,
                                  gt_q)  # gt_q won't be used in optimization if pi estimation strategy is self or uniform
        ...
        for i, class_ in enumerate(semi_val_loader.dataset.all_classes):
            if cls_union[i] == 0:
                continue
            IoU = cls_intersection[i] / (cls_union[i] + 1e-10)
            print("Class {}: \t{:.4f}".format(class_, IoU))
            if class_ in semi_val_loader.dataset.base_class_list:
                sum_base_IoU += IoU
                base_count += 1
            elif class_ in semi_val_loader.dataset.novel_class_list:
                sum_novel_IoU += IoU
                novel_count += 1
