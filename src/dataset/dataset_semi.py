...
from src.dataset.classes import get_split_classes
from src.dataset.utils import make_dataset


def get_semi_val_loader(args: argparse.Namespace) -> torch.utils.data.DataLoader:
    """
...
class MultiClassValData(Dataset):
    def __init__(self, transform: transform.Compose, base_class_list: List[int], novel_class_list: List[int],
        ...
        self.shot = args.shot
        self.cls_shot = args.cls_shot
        self.semi_shot = args.semi_shot
        self.data_root = args.data_root
    ...
    def generate_support(self, query_image_path_list, remove_them_from_query_data_list=False):
        ...
        for c in self.novel_class_list:
            ...
            for idx in indices_list:
                if len(current_path_list) >= self.semi_shot+self.cls_shot:
                    break
            ...
            if found_images_count < self.semi_shot+self.cls_shot:
        ...
        # print(support_label_path_list)

        cls_spprt_list, semi_spprt_list, cls_label_list, semi_label_list = list(),list(),list(),list()
        cls_spprt_path_list, semi_spprt_path_list, cls_label_path_list, semi_label_path_list = list(),list(),list(),list()
        novel_class_num = len(self.novel_class_list)
        for i in range(novel_class_num):
            cls_start = i*(self.cls_shot+self.semi_shot)
            semi_start = cls_start + self.cls_shot
            cls_spprt_list.extend(image_list[cls_start:semi_start])
            cls_label_list.extend(label_list[cls_start:semi_start])
            semi_spprt_list.extend(image_list[semi_start:semi_start+self.semi_shot])
            semi_label_list.extend(label_list[semi_start:semi_start+self.semi_shot])

            cls_spprt_path_list.extend(support_image_path_list[cls_start:semi_start])
            cls_label_path_list.extend(support_label_path_list[cls_start:semi_start])
            semi_spprt_path_list.extend(support_image_path_list[semi_start:semi_start+self.semi_shot])
            semi_label_path_list.extend(support_label_path_list[semi_start:semi_start+self.semi_shot])
        
        # print('cls_spprt_path_list',cls_spprt_path_list)
        # print('semi_spprt_path_list',semi_spprt_path_list)
        # print('cls_label_path_list',cls_label_path_list)
        # print('semi_label_path_list',semi_label_path_list)


        cls_transformed_image_list, cls_transformed_label_list = list(), list()
        if self.cls_shot == 1:
            for i, l in zip(cls_spprt_list, cls_label_list):
                transformed_i, transformed_l = self.transform(i, l)
                cls_transformed_image_list.append(transformed_i.unsqueeze(0))
                cls_transformed_label_list.append(transformed_l.unsqueeze(0))
        else:
            with Pool(self.shot) as pool:
                for transformed_i, transformed_l in pool.starmap(self.transform, zip(cls_spprt_list, cls_label_list)):
                    cls_transformed_image_list.append(transformed_i.unsqueeze(0))
                    cls_transformed_label_list.append(transformed_l.unsqueeze(0))
                pool.close()
                pool.join()

        semi_transformed_image_list, semi_transformed_label_list = list(), list()
        if self.semi_shot == 1:
            for i, l in zip(semi_spprt_list, semi_label_list):
                transformed_i, transformed_l = self.transform(i, l)
                semi_transformed_image_list.append(transformed_i.unsqueeze(0))
                semi_transformed_label_list.append(transformed_l.unsqueeze(0))
        else:
            with Pool(self.shot) as pool:
                for transformed_i, transformed_l in pool.starmap(self.transform, zip(semi_spprt_list, semi_label_list)):
                    semi_transformed_image_list.append(transformed_i.unsqueeze(0))
                    semi_transformed_label_list.append(transformed_l.unsqueeze(0))
                pool.close()
                pool.join()
        ...
        cls_spprt = torch.cat(cls_transformed_image_list, 0)
        cls_label = torch.cat(cls_transformed_label_list, 0)
        semi_spprt = torch.cat(semi_transformed_image_list, 0)
        semi_label = torch.cat(semi_transformed_label_list, 0)
        ...
        return cls_spprt, cls_label,semi_spprt,semi_label
